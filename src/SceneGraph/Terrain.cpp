...
void Terrain::GenerateTerrain() {
    ...
    for (int x = 0; x < mapParameters_->numCellsX; x++) {
        for (int z = 0; z < mapParameters_->numCellsZ; z++) {
            heights_[i] = 0.0f;
            if (secondColor)
                colors_[i] = Vector<float>(.3f, .45f, .15f);
            else
                colors_[i] = Vector<float>(.4f, .6f, .2f);
            i++;
    ...
    Range<float> heightRange(heights_[0]);
    for (int x = 0; x < mapParameters_->numCellsX; x++) {
        for (int z = 0; z < mapParameters_->numCellsZ; z++) {
    ...
    for (int x = -3; x <= 2; ++x) {
        for (int z = -3; z <= 2; ++z) {
            int xx = mapParameters_->numCellsX/2 + x,
                zz = mapParameters_->numCellsZ/2 + z;
            CellColor(xx, zz) = (xx + zz) % 2 ? Vector<float>(.4f, .4f, .4f) : Vector<float>(.6f, .6f, .6f);
        }
    }
    ...
    for (int x = 0; x < mapParameters_->numCellsX; x++) {
        for (int z = 0; z < mapParameters_->numCellsZ; z++) {
            ...
            if (       (CellHeight(x, z)                                   <= 0.0f)
                ...
                CellHeight(x, (zMod + 1).ToInt())                  = 0.0f;
                CellHeight((xMod + 1).ToInt(), (zMod + 1).ToInt()) = 0.0f;
                CellColor(x, z) = Vector<float>(.2f, .2f, .8f);
            }
        }
    }
}
